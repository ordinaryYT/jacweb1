/* ============================
   main.js  — ThatLegendJack
   Client-side logic for:
   - Section routing
   - Auth (optional server)
   - Twitch player
   - Spotify now playing
...
/* ---------- Small helper to call JSON APIs (optional server) ---------- */
...
async function doLogin(e) {
  ...
  // Try server login if available; otherwise just demo-success.
  try {
    await api('/api/auth/login', { method: 'POST', body: { username, password } });
    alert(`Logged in as ${username}`);
  } catch (err) {
    console.warn('Login endpoint failed or missing:', err.message);
...
function initTwitch() {
  ...
  const parentHost = location.hostname || 'thatlegendjack.onrender.com';
...
async function loadConfig() {
  try {
    const cfg = await api('/api/config');        // expects { discord_user_id: "..." }
...
/* ============================
   Gifted Subs (server-first; fallback local)
============================ */
...
async function loadSubs() {
  ...
  try {
    ...
    console.warn('Server /api/subs not available, falling back to localStorage:', e.message);
  }
...
function renderSubsTable(rows, box) {
  ...
  const sorted = [...rows].sort((a, b) => (b.gifts || b.Gifts || 0) - (a.gifts || a.Gifts || 0));
  const tr = sorted.map((s, i) =>
    `<tr><td>${i + 1}</td><td>${s.username || s.user}</td><td>${s.gifts || s.Gifts}</td></tr>`
  ).join('');
...
async function addOrUpdateSub() {
  ...
  try {
    ...
    console.warn('Server /api/subs POST not available, falling back to localStorage:', e.message);
  }
...
async function resetSubs() {
  ...
  // Try server delete for each row
  try {
    const subs = await api('/api/subs');
    for (const r of subs) {
    ...
    console.warn('Server /api/subs DELETE not available, falling back to localStorage:', e.message);
  }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Discord (via /api/config -> env)
  loadConfig();

  // Leaderboards
  loadSubs();

  // Prefill saved Twitch creds if present (for convenience)
  const idEl = document.getElementById('twitch_client_id');
  const tkEl = document.getElementById('twitch_access_token');
