// server.js
// Full backend with Postgres schema creation + API + static hosting
...
app.use(express.static(path.join(__dirname, 'public'))); // serve your frontend from /public
...
// Admin seed (can override via env)
...
async function ensureSchema() {
  ...
  try {
    ...
    `);
    await client.query(`INSERT INTO site_state (id) VALUES (TRUE) ON CONFLICT (id) DO NOTHING;`);
    ...
    `);
    await client.query(`INSERT INTO social_links (id) VALUES (TRUE) ON CONFLICT (id) DO NOTHING;`);
    ...
    `);
    await client.query(`INSERT INTO twitch_creds (id) VALUES (TRUE) ON CONFLICT (id) DO NOTHING;`);
...
app.put('/api/site', authRequired, adminOrStaff, async (req, res) => {
  ...
  await pool.query('UPDATE site_state SET about_html=$1, discord_status=$2 WHERE id=TRUE', [
...
app.put('/api/socials', authRequired, adminOrStaff, async (req, res) => {
  ...
  await pool.query(
    `UPDATE social_links
     SET twitch_url=$1, tiktok_url=$2, kick_url=$3, onlyfans_url=$4, twitch_img=$5, tiktok_img=$6, kick_img=$7
     WHERE id=TRUE`,
...
app.get('/api/twitch/bits', async (req, res) => {
  ...
  // In Node 18+, fetch is global. If on older Node, install node-fetch@3 and import it.
...
// --- Fallback: serve index for root (optional) ---
app.get('/', (req, res) => {
